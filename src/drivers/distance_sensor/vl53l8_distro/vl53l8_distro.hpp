...
private:
	...
	// Próba zdekodowania jednej ramki z ring buffera (bez czekania)
	bool try_read_packet(PacketType &packet_type);
	...
	// --- TX scheduling (attitude 50Hz, timesync 1Hz) ---
	...
	static inline void advance_deadline(hrt_abstime &t, hrt_abstime period, hrt_abstime now)
	{
		t += period;
		if ((int64_t)(now - t) >= 0) {
			const hrt_abstime n = (now - t) / period + 1;
			t += n * period;
		}
	}
	...
	// uORB input for attitude -> UART
	uORB::Subscription _att_sub{ORB_ID(vehicle_attitude)};
	vehicle_attitude_s _att_last{};
	bool _att_have_last{false};
